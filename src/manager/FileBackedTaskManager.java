...
3,SUBTASK,Sub Task2,DONE,Description sub task3,2 */
    private String toString(Task task){
        ...
        String res = String.format("%d, %s, %s, %s, %s, %s%n",
                task.getId(), type, task.getTitle(), task.getStatus(), task.getDescription(), epicId==-1?"":epicId);
        return res;
    }
    ...
    private static Task fromString(String taskStr){
        ...
        switch (TaskTypes.toType(taskArray[1])){
            case TASK: Task task = new Task(taskArray[2], taskArray[4], Status.toStatus(taskArray[3]));
            task.setId(Integer.parseInt(taskArray[0]));
            return task;
            case EPIC: Epic epic = new Epic(taskArray[2], taskArray[4], Status.toStatus(taskArray[3]));
                epic.setId(Integer.parseInt(taskArray[0]));
                return epic;
            case SUBTASK: Subtask subtask = new Subtask(taskArray[2], taskArray[4], Status.toStatus(taskArray[3]),
                    Integer.parseInt(taskArray[5]));
